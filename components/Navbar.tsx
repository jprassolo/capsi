...
const Navbar: React.FC<NavbarProps> = ({ onNavClick }) => {
  ...
  const navClasses = scrolled || mobileMenuOpen 
    ? 'bg-[#F5F2EB]/95 backdrop-blur-md py-4 shadow-sm text-[#2C2A26]' 
    : 'bg-transparent py-8 text-[#2C2A26]';

  return (
    <>
      <nav 
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ease-in-out ${navClasses}`}
      >
        <div className="max-w-[1800px] mx-auto px-8 flex items-center justify-between">
          ...
          <a 
            ...
            className={`text-2xl md:text-3xl font-serif font-medium tracking-tight z-50 relative transition-colors duration-500`}
          >
            {PROFILE.name}
          </a>
          
          {/* Center Links - Desktop */}
          <div className={`hidden md:flex items-center gap-12 text-sm font-medium tracking-widest uppercase transition-colors duration-500`}>
            <a href="#products" onClick={(e) => handleLinkClick(e, 'products')} className="hover:opacity-60 transition-opacity">Servicios</a>
